...
#define SAFE_DEVICE_NAME "/dev/nvme0n1p1"
#define MB 1024 * 1024
...
int _nvme_io(int fd, struct nvme_user_io* io) {
...
void nvme_io(int fd, OPCODE ioType, char* buffer, const size_t bufferSize) {
    ...
    while (remainBlocks > 0) {
        ...
        io.nblocks = (operateBlocks - 1);
        io.addr = (__u64)(currentBuffer);
        io.slba = currentLba;
        error = _nvme_io(fd, &io) < 0;
        offsetBytes += operateBlocks * NVME_SECTOR_SIZE;
        currentLba += operateBlocks;
        remainBlocks -= operateBlocks;
    }
    check_error(error, "Failed to submit IO");
}
