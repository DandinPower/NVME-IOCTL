		...
		unsigned int flags)
{
	...
	if (ioucmd && (ioucmd->flags & IORING_URING_CMD_FIXED)) {
		// LIAW ADD START
		printk(KERN_INFO "LIAW: nvme_map_user_request: ioucmd && (ioucmd->flags & IORING_URING_CMD_FIXED)\n");
		// LIAW ADD END
		...
		if (WARN_ON_ONCE(flags & NVME_IOCTL_VEC)) {
			// LIAW ADD START
			printk(KERN_INFO "LIAW: nvme_map_user_request: WARN_ON_ONCE(flags & NVME_IOCTL_VEC)\n");
			// LIAW ADD END
			return -EINVAL;
		}
		...
		if (ret < 0) {
			// LIAW ADD START
			printk(KERN_INFO "LIAW: nvme_map_user_request: io_uring_cmd_import_fixed: ret=%d\n", ret);
			// LIAW ADD END
			goto out;
		}
		...
		// LIAW ADD START
		printk(KERN_INFO "LIAW: nvme_map_user_request: blk_rq_map_user_iov: ret=%d\n", ret);
		// LIAW ADD END
	} else {
		ret = blk_rq_map_user_io(req, NULL, nvme_to_user_ptr(ubuffer),
				...
				0, rq_data_dir(req));
		// LIAW ADD START
		printk(KERN_INFO "LIAW: nvme_map_user_request: blk_rq_map_user_io: ret=%d\n", ret);
		// LIAW ADD END
	}
	...
	if (bdev && meta_buffer && meta_len) {
		...
		if (IS_ERR(meta)) {
			ret = PTR_ERR(meta);
			// LIAW ADD START
			printk(KERN_INFO "LIAW: nvme_map_user_request: nvme_add_user_metadata: ret=%d\n", ret);
			// LIAW ADD END
			goto out_unmap;
		}
		*metap = meta;
...
out:
	...
	// LIAW ADD START
	printk(KERN_INFO "LIAW: nvme_map_user_request: end: ret=%d\n", ret);
	// LIAW ADD END
	return ret;
}
...
{
	// LIAW ADD START
	printk(KERN_INFO "LIAW: nvme_submit_user_cmd: start\n");
	// LIAW ADD END
	...
	if (ubuffer && bufflen) {
		...
		if (ret) {
			// LIAW ADD START
			printk(KERN_INFO "LIAW: nvme_submit_user_cmd: nvme_map_user_request\n");
			// LIAW ADD END
			return ret;
		}
	}
	...
	// LIAW ADD START
	if (ret)
		printk(KERN_INFO "LIAW: nvme_submit_user_cmd: ret\n");
	// LIAW ADD END
	...
	// LIAW ADD START
	printk(KERN_INFO "LIAW: nvme_submit_user_cmd: end\n");
	// LIAW ADD END
	return ret;
}
...
{
	// LIAW ADD START
	printk(KERN_INFO "LIAW: nvme_submit_io: start\n");
	// LIAW ADD END
	...
	if (io.flags) {
		// LIAW ADD START
		printk(KERN_INFO "LIAW: nvme_submit_io: io.flags\n");
		// LIAW ADD END
		return -EINVAL;
	}
	...
	default:
		// LIAW ADD START
		printk(KERN_INFO "LIAW: nvme_submit_io: default\n");
		// LIAW ADD END
		return -EINVAL;
	}
	...
	if ((io.control & NVME_RW_PRINFO_PRACT) &&
		...
		if (nvme_to_user_ptr(io.metadata)) {
			// LIAW ADD START
			printk(KERN_INFO "LIAW: nvme_submit_io: nvme_to_user_ptr(io.metadata)\n");
			// LIAW ADD END
			return -EINVAL;
		}
		meta_len = 0;
	...
	if (ns->features & NVME_NS_EXT_LBAS) {
		...
		if ((io.metadata & 3) || !io.metadata) {
			// LIAW ADD START
			printk(KERN_INFO "LIAW: nvme_submit_io: (io.metadata & 3) || !io.metadata\n");
			// LIAW ADD END
			return -EINVAL;
		}
	}
	...
	// LIAW ADD START
	printk(KERN_INFO "LIAW: nvme_submit_io: go to nvme_submit_user_cmd\n");
	// LIAW ADD END
		...
		fmode_t mode)
{
	...
	if (cmd.flags) {
		// LIAW ADD START
		printk(KERN_INFO "LIAW: nvme_user_cmd: cmd.flags\n");
		// LIAW ADD END
		return -EINVAL;
	}
	if (!nvme_validate_passthru_nsid(ctrl, ns, cmd.nsid)) {
		// LIAW ADD START
		printk(KERN_INFO "LIAW: nvme_user_cmd: !nvme_validate_passthru_nsid(ctrl, ns, cmd.nsid)\n");
		// LIAW ADD END
		return -EINVAL;
	}
		...
		fmode_t mode)
{
	...
	if (cmd.flags) {
		// LIAW ADD START
		printk(KERN_INFO "LIAW: nvme_user_cmd64: cmd.flags\n");
		// LIAW ADD END
		return -EINVAL;
	}
	if (!nvme_validate_passthru_nsid(ctrl, ns, cmd.nsid)) {
		// LIAW ADD START
		printk(KERN_INFO "LIAW: nvme_user_cmd64: !nvme_validate_passthru_nsid(ctrl, ns, cmd.nsid)\n");
		// LIAW ADD END
		return -EINVAL;
	}
		...
		struct io_uring_cmd *ioucmd, unsigned int issue_flags, bool vec)
{
	...
	if (c.common.flags) {
		// LIAW ADD START
		printk(KERN_INFO "LIAW: nvme_uring_cmd_io: c.common.flags\n");
		// LIAW ADD END
		return -EINVAL;
	}
	...
	if (!nvme_validate_passthru_nsid(ctrl, ns, le32_to_cpu(c.common.nsid))) {
		// LIAW ADD START
		printk(KERN_INFO "LIAW: nvme_uring_cmd_io: !nvme_validate_passthru_nsid(ctrl, ns, le32_to_cpu(c.common.nsid))\n");
		// LIAW ADD END
		return -EINVAL;
	}
