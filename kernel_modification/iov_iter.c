...
// LIAW ADD START
static int want_pages_array_contiguous(struct page ***res, size_t size,
			    size_t start, unsigned int maxpages)
{
	unsigned int count = DIV_ROUND_UP(size + start, PAGE_SIZE);
	...
	WARN_ON(!count);	// caller should've prevented that
	if (!*res) {
		*res = kmalloc_array(count, sizeof(struct page *), GFP_KERNEL);
		if (!*res)
			return 0;
	}
	return count;
		   ...
		   unsigned int gup_flags)
{
	...
	if (likely(user_backed_iter(i))) {
		...
		// LIAW ADD START
		printk(KERN_INFO "LIAW: iov_iter_get_pages_alloc: user_backed_iter\n");
		// LIAW ADD END
...
// LIAW ADD START
static ssize_t __iov_iter_get_pages_alloc_contiguous(struct iov_iter *i,
		   struct page ***pages, size_t maxsize,
		   unsigned int maxpages, size_t *start,
		   unsigned int gup_flags)
{
	...
	if (likely(user_backed_iter(i))) {
		...
		*start = addr % PAGE_SIZE;
		...
		// LIAW ADD START
		printk(KERN_INFO "LIAW: iov_iter_get_pages_alloc: addr = %lu, *start = %lu\n", addr, *start);
		// LIAW ADD END

		addr &= PAGE_MASK;
		n = want_pages_array_contiguous(pages, maxsize, *start, maxpages);
		if (!n)
			return -ENOMEM;
		res = get_user_pages_fast(addr, n, gup_flags, *pages);
		if (unlikely(res <= 0))
...
// LIAW ADD START
ssize_t iov_iter_get_pages_alloc_contiguous(struct iov_iter *i,
		   struct page ***pages, size_t maxsize,
		   size_t *start, unsigned gup_flags)
{
	ssize_t len;
	...
	*pages = NULL;
	...
	len = __iov_iter_get_pages_alloc_contiguous(i, pages, maxsize, ~0U, start,
					 gup_flags);
	if (len <= 0) {
		kvfree(*pages);
		*pages = NULL;
...
EXPORT_SYMBOL_GPL(iov_iter_get_pages_alloc_contiguous);
// LIAW ADD END
